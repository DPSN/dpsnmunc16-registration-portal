...
<html>
  <head>
    ...
    <body>
        ...
        <h2>Photography and Videography Applicants</h2>
        <form method="POST" action="savechanges.php">
            <table>
                <tr>
                    <th>Name</th><th>Email</th><th>Phone</th><th>Date of Birth</th><th>Institution Name</th><th>Workshops</th><th>Events Attended</th><th>Achievements</th><th>More</th><th>Confirmation</th>
                </tr>
        <?php
        include_once("../db.php");
        $sql = "SELECT * FROM `photoapp`;";
        ...
        while($ar = mysqli_fetch_array($res)) {
            ...
            $workshopexp = $ar['workshopexp'];
            $eventexp = $ar['eventexp'];
            $achieved = $ar['achievement'];
            $else = $ar['else'];
            $confirm = $ar['confirmed'];
            print("<tr><td>$name</td><td><a href=\"mailto:$email\">$email</a></td><td>$number</td><td>$dob</td><td>$insti</td><td>$workshopexp</td><td>$eventexp</td><td>$achieved</td><td>$else</td>");
            if($confirm == "yes") {
                print("<td>");
